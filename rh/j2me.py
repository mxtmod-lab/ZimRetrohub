...
def sync_bundled_runtime_files():
    ...
    if not os.path.isdir(bundled_root):
        return []
    # Chi dong bo cap nhat khi gia lap Java da duoc cai dat thuc su tren the (co JRE zulu17/bin/java)
    # Tranh tao thu muc Emus/JAVA rong gay hieu nham la da co gia lap
    if not os.path.isfile(f"{RUNTIME_DIR}/bin/java") and not has_payload():
        return []
    updated = []
...
def ensure_latest_j2me_installed():
    ...
    # Neu chua cai dat Java va khong co payload cuc bo, khong tu dong tao thu muc cau hinh
    if not os.path.exists(f"{RUNTIME_DIR}/bin/java") and not has_payload():
        return True, None
