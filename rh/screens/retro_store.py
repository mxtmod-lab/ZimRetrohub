...
class RetroStoreScreen(BaseScreen):
    """4x2 Grid Retro Store Hub containing Game Store, Emu Store, App Store, Theme Store, Icon Store, Cheat Code, Boxart Scraper."""
    ...
    def on_enter(self, params=None):
        self.items = [
            ...
            {
                "id": "item_apps",
                "title": tr("retro_store_apps"),
                "sub": tr("retro_store_apps_sub"),
                "icon": os.path.join(APP_DIR, "assets", "ic-apps.png"),
                "action": "screen:app_store",
                "badge": "Apps"
            },
            {
                "id": "item_themes",
    ...
    def get_footer_actions(self):
        return [
            ...
            ("Y", tr("dlq_open"), (255, 200, 0), (220, 225, 235), True),
    ...
    def handle_input(self, inputs):
        ...
        # Y: hang cho tai (mo tu bat ky tang nao cua Retro Store).
        if inputs.get("btn_y"):
            self.engine.push_screen("downloads")
            return True
        ...
        if btn_left:
            if col > 0:
                ...
                row_end = min(num_items - 1, row * self.COLS + (self.COLS - 1))
        ...
        if btn_down:
            if row < self.ROWS - 1:
                ...
                self.selected_idx = min(col, num_items - 1)
            return True
        ...
        if btn_a and 0 <= self.selected_idx < num_items:
            ...
            elif act == "screen:app_store":
                self.engine.push_screen("app_store")
