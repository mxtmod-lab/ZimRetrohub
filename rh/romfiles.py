...
ROM_EXT_PRIORITY = {
    ...
    "PS1": [".m3u", ".cue", ".chd", ".pbp"],
    "PSP": [".iso", ".cso"],
    "DC":  [".chd", ".cdi", ".gdi", ".cue", ".iso", ".zip"],
}
GENERIC_ROM_EXTS = [".m3u", ".cue", ".gdi", ".chd", ".cdi", ".iso", ".gba", ".sfc", ".smc", ".nes",
...
def referenced_by_sidecar(rom_path, companions):
    """True khi mot .cue/.m3u/.gdi ben canh tro dung ten file cua *rom_path*.

    Doi ten mot .bin/.iso/.raw ma co .cue/.gdi tro toi no thi sidecar tro vao khoang khong.
    Doi ten chinh cai sidecar thi khong sao: sidecar tro ten file con chu khong tu tro minh.
    """
    base = os.path.basename(rom_path).lower()
    for c in companions:
        if not c.lower().endswith((".cue", ".m3u", ".gdi")):
            continue
        try:
