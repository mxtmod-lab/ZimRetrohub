...
def get_emus_status():
    ...
    for item in catalog:
        ...
        if sys_id == "JAVA":
            # Rieng JAVA la standalone runtime: bat buoc phai co file binary java thuc su
            java_bin = os.path.join(emu_path, "zulu17", "bin", "java")
            is_installed = os.path.isfile(java_bin)
        else:
...
def install_emu(sys_id):
    ...
    if not target_archive:
        ...
        urls_to_try = [
            f"{ONLINE_CDN_BASE}emus/{tar_name}",
