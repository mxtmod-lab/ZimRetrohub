...
from ..boxart import is_real_boxart_url, no_art_known
...
from ..downloader import (enqueue_download, dl_state, game_key, download_state_for,
                         ...
                         resume_active_download, queue_position)
...
class GameActionModal(BaseModal):
    ...
    def _fetch_boxart_auto(self):
        ...
        def _bg():
            try:
                ...
                # Da do het chuoi ma khong ra bia (cache nay do chinh scraper ghi)
                # thi thoi, khong bat nguoi dung cho lai 10-40s moi lan mo chi tiet.
                if no_art_known(sys_code, fname):
                    return
                    ...
                    if ok and os.path.exists(target_art):
                        ...
                        if g_title and g_title != rom_base:
                            try:
                                ...
                                pass
                    # URL bia that nhung tai khong duoc thi dung: chay tiep chuoi
                    # do cua scraper la tai hai lan cho cung mot game.
                else:
                    # URL chi la anh "chua co bia" (placeholder) nen phai do that.
    ...
    def is_paused(self):
        """Phien cua dung game nay dang tam dung (co the bam A de chay tiep)."""
        return download_state_for(self.game_info or {}) == "paused"

    def is_cancelling(self):
        """Phien cua dung game nay da duoc yeu cau huy, dang cho worker tra loi.

        So theo game_key chu khong theo title: hai he may co the co game trung ten."""
        if not self.game_info:
            return False
        return (dl_state.get("status") == "cancelling"
                and game_key(dl_state.get("game_info")) == game_key(self.game_info))

    def is_queued(self):
        """Game nay dang nam trong hang cho, chua toi luot tai."""
        return download_state_for(self.game_info or {}) == "queued"

    def is_downloading(self):
        """Phien cua dung game nay dang chay: tai, tam dung, hoac dang huy.

        Khong tinh "queued": muc dang cho ma ve tien do thi no dang ve so lieu cua
        mot game khac dang tai."""
        if not self.game_info:
            return False
        return download_state_for(self.game_info) in ("downloading", "cancelling", "paused")
    ...
    def handle_input(self, inputs):
        ...
        if self.is_downloading():
            if btn_b:
                ...
                self.close()
                return True
            if btn_a and self.is_paused():
                resume_active_download()
                if self.engine:
                    self.engine.toast(tr("dlq_resumed_toast"))
                return True
            if btn_x:
                if self.is_cancelling():
                    # Da yeu cau huy roi: bam them chi to them mot lan huy nua.
                    return True
                was_running = dl_state.get("status") in ("downloading", "cancelling")
        ...
        # ----------------------------------------------------
        # 2. State: Queued (dang cho toi luot, khong co so lieu tien do nao ca)
        # ----------------------------------------------------
        if self.is_queued():
            if btn_b:
                self.close()
            ...
            if btn_x:
                if cancel_download(self.game_info):
                    self.was_downloading = False
                    if self.engine:
                        self.engine.toast(tr("dl_cancel_queued"))
                return True
            return True

        # ----------------------------------------------------
        # 3. State: Pre-Download (Not yet downloaded)
        # ----------------------------------------------------
    ...
    def render(self, engine):
        ...
        if is_dling:
            if self.is_cancelling():
                header_str = tr("dlq_status_cancelling")
            elif self.is_paused():
                header_str = tr("dlq_status_paused")
            else:
                header_str = tr("dl_header_progress")
        elif self.is_queued():
            header_str = tr("dlq_status_queued")
        elif not is_dl:
        ...
        if self.is_queued() and not is_dling:
            # Dang cho: khong co gi de xem, chi con rut khoi hang cho hoac dong.
            engine.draw_footer_btn(32, fy, foot_h, "X", tr("dlq_cancel_queued_btn"),
                                   (255, 75, 75), is_dark_btn=True)
            engine.draw_footer_btn(state.SCREEN_W - 165, fy, foot_h, "B",
                                   tr("dl_footer_close"), (255, 75, 75), is_dark_btn=False)
        elif is_dling:
            if self.is_cancelling():
                # Dang huy: het viec de bam, chi con dong man hinh.
                engine.draw_footer_btn(state.SCREEN_W - 165, fy, foot_h, "B",
                                       tr("dl_footer_close"), (255, 75, 75), is_dark_btn=False)
            elif self.is_paused():
                # Tam dung: tra lai dung hai viec con lam duoc.
                engine.draw_footer_btn(32, fy, foot_h, "A", tr("dlq_footer_resume"),
                                       (0, 230, 150), is_dark_btn=True)
                engine.draw_footer_btn(state.SCREEN_W - 175, fy, foot_h, "X",
                                       tr("dl_footer_cancel"), (255, 75, 75), is_dark_btn=True)
            else:
        ...
        # dung ca chieu cao con lai cua panel phai. Muc dang cho cung co card rieng
        # ben duoi (vi tri trong hang cho), nen phai tinh nhu luc dang tai.
        if is_dling or is_dl or self.is_queued():
        ...
        # CASE A2: QUEUED - muc dang cho toi luot: khong co so lieu tien do nao de
        # ve, va so lieu dang co trong dl_state la cua game khac dang tai.
        elif self.is_queued():
            engine.fill_rect(right_x, bottom_y, right_w, bottom_h, 18, 25, 42, 255)
            engine.draw_rect(right_x, bottom_y, right_w, bottom_h, 255, 200, 0, 255, thickness=2)
            pos = queue_position(self.game_info) or 0
            engine.fill_rect(right_x + 2, bottom_y + 2, right_w - 4, 44, 40, 34, 16, 255)
            engine.draw_text(tr("dlq_status_queued"), engine.font_sub, right_x + 18, bottom_y + 24,
                             255, 215, 0, center_y=True, max_w=right_w - 36)
            lines = engine.wrap_text_to_width(
                tr("dlq_queue_position").format(n=pos), engine.font_item, right_w - 48, max_lines=2)
            for i, line in enumerate(lines):
                engine.draw_text(line, engine.font_item, right_x + 24, bottom_y + 92 + i * 34,
                                 235, 240, 250, max_w=right_w - 48)
            engine.draw_text(tr("dlq_queue_hint"), engine.font_sub, right_x + 24,
                             bottom_y + 176, 180, 200, 230, max_w=right_w - 48)
