...
def _is_script_broken(emu_dir, script_path):
    ...
    # Neu la gia lap Java J2ME ma thieu binary java hoac freej2me-sdl.jar thi coi la broken
    if "freej2me" in content or "zulu17" in content:
        java_bin = os.path.join(emu_dir, "zulu17", "bin", "java")
        jar_file = os.path.join(emu_dir, "zulu17", "bin", "freej2me-sdl.jar")
        if not (os.path.isfile(java_bin) and os.path.isfile(jar_file)):
            return True
