...
def pick_font(candidates, has_viet):
    ...
    """
    cached_font_file = "/tmp/.rh_font"
    try:
        if os.path.isfile(cached_font_file):
            with open(cached_font_file, "r", encoding="utf-8") as f:
                cf = f.read().strip()
            if cf and os.path.isfile(cf) and has_viet(cf):
                return cf
    except Exception:
        pass
    ...
    for cand in candidates:
        ...
        if ok:
            try:
                with open(cached_font_file, "w", encoding="utf-8") as f:
                    f.write(cand)
            except Exception:
                pass
            return cand
