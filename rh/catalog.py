    ...
    for sys_code in sorted(sys_dirs):
        ...
        try:
            for scan_dir in scan_dirs:
                # Thu thap cac track con bi sidecar (.cue/.gdi/.m3u) tham chieu de khong hien duplicate / rac
                referenced_tracks = set()
                try:
                    for item in os.listdir(scan_dir):
                        item_lower = item.lower()
                        if item_lower.endswith((".cue", ".gdi", ".m3u")) and not item.startswith("."):
                            try:
                                with open(os.path.join(scan_dir, item), "r", encoding="utf-8", errors="ignore") as sf:
                                    for line in sf:
                                        for token in line.split():
                                            clean_tok = token.strip('"\';,').lower()
                                            if clean_tok.endswith((".bin", ".raw", ".iso", ".img", ".wav")):
                                                referenced_tracks.add(os.path.basename(clean_tok))
                            except OSError:
                                pass
                except OSError:
                    pass
                ...
                with os.scandir(scan_dir) as entries:
                    for entry in sorted(entries, key=lambda e: e.name.lower()):
                        ...
                        if name.startswith("."):
                            continue
                        if name_lower in referenced_tracks:
                            continue
                        if actual_code == "DC" and name_lower.endswith((".bin", ".raw")):
                            continue
