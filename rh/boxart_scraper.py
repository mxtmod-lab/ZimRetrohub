...
from .boxart import is_real_boxart_url, remember_no_art, forget_no_art
...
def get_libretro_file_list(sys_folder, category="Named_Boxarts", allow_fetch=True):
    ...
    # De tren the: /tmp la tmpfs, reboot mot cai la mat danh sach, va he may dau
    # tien sau moi lan khoi dong lai phai cho 3-15s de tai lai.
    cache_dir = os.path.join(SDCARD_PATH, ".retrohub", "cache")
    try:
        os.makedirs(cache_dir, exist_ok=True)
        tmp_path = os.path.join(cache_dir, f"libretro_{safe_name}.json")
    except OSError:
...
def scrape_boxart_for_single_rom(item_or_sys, filename="", title="", rom_path="", base_sd=None):
    ...
    if not best_url:
        # Chi ghi nho khi da do het ma khong co URL nao: tai that bai vi mang chap
        # chon thi khong duoc phep an bia cua game trong 7 ngay.
        remember_no_art(sys_code, fname)
    if best_url:
