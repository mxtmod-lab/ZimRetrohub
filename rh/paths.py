...
# ------------------------------------------------------------------------------
# Apps Store & Packages Paths
# ------------------------------------------------------------------------------
APPS_DIR = os.path.join(SDCARD_PATH, "Apps")
APPS_CATALOG_FILE = os.path.join(APP_DIR, "catalog", "apps_catalog.json")

def _detect_apps_packages_dir():
    candidates = [
        os.path.join(SDCARD_PATH, "apps"),
        os.path.join(SDCARD_PATH, "Apps_repo"),
        os.path.join(APP_DIR, "apps"),
        os.path.join(os.path.dirname(APP_DIR), "apps"),
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "apps"),
    ]
    for cand in candidates:
        if os.path.isdir(cand):
            return cand
    return os.path.join(SDCARD_PATH, "apps")

LOCAL_APPS_PACKAGES_DIR = _detect_apps_packages_dir()
