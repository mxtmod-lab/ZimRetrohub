...
from .screens.lazy import LazyScreenMap
...
class RetroHubEngine:
    ...
    def __init__(self):
        ...
        # Screen management
        self.screens = LazyScreenMap(self)
        self.screen_stack = []
    ...
    # --------------------------------------------------------------------------
    def register_screen(self, name, screen_or_module, class_name=None):
        if class_name:
            self.screens.register_lazy(name, screen_or_module, class_name)
        else:
            screen_or_module.engine = self
            self.screens[name] = screen_or_module

    def register_lazy_screen(self, name, module_name, class_name):
        self.screens.register_lazy(name, module_name, class_name)
    ...
    def run(self):
        ...
        # Tu dong kiem tra va cai dat toan bo gia lap + PortMaster lan dau mo app
        if not getattr(state, "initial_setup_done", False):
            def _bg_initial_setup_check():
                import time as _t
                _t.sleep(1.0)
                try:
                    from .emulator_store import get_emus_status
                    from .app_store import get_apps_status
                    from .modals.initial_setup import InitialSetupModal

                    missing_emus = [
                        {"type": "emu", "id": it.get("id"), "name": it.get("name", it.get("id")), "category": it.get("category", "")}
                        for it in (get_emus_status() or [])
                        if not it.get("installed")
                    ]

                    apps = get_apps_status() or []
                    pm = next((a for a in apps if a.get("id") == "PortMaster"), None)
                    missing_apps = []
                    if pm and not pm.get("installed"):
                        missing_apps.append({
                            "type": "app",
                            "id": "PortMaster",
                            "name": "PortMaster",
                    ...
                    all_missing = missing_emus + missing_apps
                    if all_missing:
                        if self.running:
                            modal = InitialSetupModal(self)
                            self.open_modal(modal, {"items": all_missing})
                    else:
                        state.initial_setup_done = True
                        state.save_settings()
                except Exception as e:
                    print(f"[ENGINE] Initial setup check error: {e}")

            import threading as _th
            _th.Thread(target=_bg_initial_setup_check, daemon=True).start()
        ...
        # Render initial frame immediately so display shows up instantly (< 300ms)
        try:
        ...
        # Background byte-compile warmup (so subsequent screen opens are also instant)
        def _bg_compile_warmup():
            import time as _t
            _t.sleep(4.0)
            try:
                import compileall
                from .paths import APP_DIR as _app
                compileall.compile_dir(_app, maxlevels=3, quiet=1)
            except Exception:
                pass

        threading.Thread(target=_bg_compile_warmup, daemon=True).start()
